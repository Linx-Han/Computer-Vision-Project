...
class Nutrition5kDataset(Dataset):
    """Dataset class for Nutrition5k with 5-channel input and enhanced augmentation"""
        ...
        try:
            ...
            if self.is_train:
                ...
                if np.random.rand() < 0.5:
                    all_channels = transforms.functional.hflip(all_channels)
                
                # 2. Random rotation (70% chance, increased from 50%)
                ...
                # 3. Random crop and resize (70% chance, increased from 50%)
                if np.random.rand() < 0.7:
                    i, j, h, w = transforms.RandomResizedCrop.get_params(
                        all_channels, scale=Config.CROP_SCALE, ratio=(0.95, 1.05)  # Slightly tighter ratio
                    )
                ...
                # 4. Color jitter (RGB only, 60% chance, increased from 50%)
                if np.random.rand() < 0.6:
                    color_jitter = transforms.ColorJitter(
                        brightness=Config.COLOR_JITTER_BRIGHTNESS,
                        contrast=Config.COLOR_JITTER_CONTRAST,
                        saturation=0.2,  # NEW: Add saturation jitter
                        hue=0.1          # NEW: Add slight hue jitter
                    )
                    rgb_pil = transforms.ToPILImage()(rgb_tensor)
                    rgb_tensor = self.to_tensor(color_jitter(rgb_pil))
                
                # 5. Random Gaussian blur (30% chance) - NEW!
                if np.random.rand() < 0.3:
                    kernel_size = np.random.choice([3, 5])  # Small blur kernels
                    sigma = np.random.uniform(0.1, 1.0)
                    rgb_pil = transforms.ToPILImage()(rgb_tensor)
                    blurred = transforms.GaussianBlur(kernel_size=kernel_size, sigma=sigma)(rgb_pil)
                    rgb_tensor = self.to_tensor(blurred)
                
                # 6. Random brightness adjustment on depth/height (20% chance) - NEW!
                # Simulates lighting variations affecting depth sensor
                if np.random.rand() < 0.2:
                    brightness_factor = np.random.uniform(0.9, 1.1)
                    depth_tensor = depth_tensor * brightness_factor
                    height_tensor = height_tensor * brightness_factor
...
if __name__ == '__main__':
    ...
    print("\n✅ Data loading successful with enhanced augmentation!")